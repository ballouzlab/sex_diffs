...
geneLoc_df_subPARY <- fread("data/subPARY.tsv") %>% filter(par=="PAR2")
...
fwrite(log_exprs_df_par1,paste0(out.dir2,"/", cellLabel,'_male_par2_expression.tsv'),sep="\t")
...
fwrite(gene_snp_df,paste0(out.dir2,"/", cellLabel,'_male_par2_gene-snp_pairs.tsv'),sep="\t")
...
fwrite(gene_snp_df,paste0(out.dir2,"/", cellLabel,'_male_par2_log_residuals.tsv'),sep="\t")

genotype_df <- genotype_df %>% filter(sampleid %in% log_exprs_df_par1$sampleid)

fwrite(genotype_df,paste0(out.dir2,"/", cellLabel,'_male_par2_genotypes.tsv'),sep="\t")
...
fwrite(log_spearman_df,sprintf("%s/%s_male_par2_correlation_results.tsv", out.dir2, cellLabel),sep="\t",quote=F)
...
fwrite(log_spearman_df_significant,sprintf("%s/%s_male_par2_significant_correlation_results.tsv", out.dir2, cellLabel),sep="\t",quote=F)
