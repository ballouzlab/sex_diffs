...
female_gene_snp_pairs <- fread(sprintf("%s/PAR1-XX/%s/%s_female_par1_gene-snp_pairs.tsv", output, cellLabel, cellLabel))
male_gene_snp_pairs <- fread(sprintf("%s/PAR1-XY/%s/%s_male_par1_gene-snp_pairs.tsv", output,cellLabel,cellLabel))
...
out.dir = paste0(output,'/PAR1-XXY/')
dir.create(out.dir)
...
fwrite(log_exprs_df_par1,paste0(out.dir2,"/", cellLabel,'_joint_par1_expression.tsv'),sep="\t")

fwrite(gene_snp_df,paste0(out.dir2,"/", cellLabel,'_joint_par1_gene-snp_pairs.tsv'),sep="\t")
...
fwrite(gene_snp_df,paste0(out.dir2,"/", cellLabel,'_joint_par1_log_residuals.tsv'),sep="\t")

genotype_df <- genotype_df %>% filter(sampleid %in% log_exprs_df_par1$sampleid)

fwrite(genotype_df,paste0(out.dir2,"/", cellLabel,'_joint_par1_genotypes.tsv'),sep="\t")
...
fwrite(log_spearman_df,sprintf("%s/%s_joint_par1_correlation_results.tsv", out.dir2, cellLabel),sep="\t",quote=F)
...
fwrite(log_spearman_df_significant,sprintf("%s/%s_joint_par1_significant_correlation_results.tsv", out.dir2, cellLabel),sep="\t",quote=F)
