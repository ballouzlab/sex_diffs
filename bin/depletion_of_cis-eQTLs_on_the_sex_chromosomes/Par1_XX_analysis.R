...
geneLoc_df_subPARX <- fread("data/subPARX.tsv") %>% filter(par=="PAR1")
...
fwrite(log_exprs_df_parx,paste0(out.dir2,"/", cellLabel,'_female_par1_expression.tsv'),sep="\t")
...
fwrite(gene_snp_df,paste0(out.dir2,"/", cellLabel,'_female_par1_gene-snp_pairs.tsv'),sep="\t")
...
fwrite(gene_snp_df,paste0(out.dir2,"/", cellLabel,'_female_par1_log_residuals.tsv'),sep="\t")

genotype_df <- genotype_df %>% filter(sampleid %in% log_exprs_df_parx$sampleid)

fwrite(genotype_df,paste0(out.dir2,"/", cellLabel,'_female_par1_genotypes.tsv'),sep="\t")
...
fwrite(log_spearman_df,sprintf("%s/%s_female_par1_correlation_results.tsv", out.dir2, cellLabel),sep="\t",quote=F)
...
fwrite(log_spearman_df_significant,sprintf("%s/%s_female_par1_significant_correlation_results.tsv", out.dir2, cellLabel),sep="\t",quote=F)
