...
# Input files
genotype_df <- fread("data/XY/chrX_PAR2_genotypes.tsv")
snpLoc_df <- fread("data/XY/chrX_PAR2.bim")
geneLoc_df_subPARX <- fread("data/subPARX.tsv") %>% filter(par=="PAR2")
...
covariate_df_f <- covariate_df %>% filter(sex==2) 
dim(covariate_df_f)

set.seed(1234)
covariate_df_f = sample_n (covariate_df, no_males)
dim(covariate_df_f)

fwrite(covariate_df_f,paste0(out.dir2,'/',cellLabel,'_female_covariates.tsv'),sep="\t")

# Separate expression file for females

log_exprs_df <- expression_df %>% filter(sampleid %in% covariate_df_f$sampleid)
dim(log_exprs_df)

# Extract PAR X region genes 
log_exprs_df_parx <- log_exprs_df %>% select("sampleid",colnames(log_exprs_df)[(colnames(log_exprs_df) %in% geneLoc_df_subPARX$geneid) == TRUE])
dim(log_exprs_df_parx)

fwrite(log_exprs_df_parx,paste0(out.dir2,"/", cellLabel,'_female_par2_expression.tsv'),sep="\t")
...
geneLoc_df <- geneLoc_df_subPARX[(geneLoc_df_subPARX$geneid %in% gene_ids),]
...
fwrite(gene_snp_df,paste0(out.dir2,"/", cellLabel,'_female_par2_gene-snp_pairs.tsv'),sep="\t")
...
calculate_residuals <- function (x) {
  ...
  test_df <- left_join(exprs_val,covariate_df_f,by="sampleid")
...
fwrite(gene_snp_df,paste0(out.dir2,"/", cellLabel,'_female_par2_log_residuals.tsv'),sep="\t")

genotype_df <- genotype_df %>% filter(sampleid %in% log_exprs_df_parx$sampleid)

fwrite(genotype_df,paste0(out.dir2,"/", cellLabel,'_female_par2_genotypes.tsv'),sep="\t")
...
fwrite(log_spearman_df,sprintf("%s/%s_female_par2_correlation_results.tsv", out.dir2, cellLabel),sep="\t",quote=F)
...
fwrite(log_spearman_df_significant,sprintf("%s/%s_female_par2_significant_correlation_results.tsv", out.dir2, cellLabel),sep="\t",quote=F)
