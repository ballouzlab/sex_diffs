...
cor_files <- list.files(path=input.dir, pattern=paste0(cellLabel,"_nonPAR_XaXi_correlation_results*"), full.names=T)
...
fwrite(log_spearman_df_significant,sprintf("%s/%s_nonPAR_XaXi_significant_correlation_results.tsv", input.dir, cellLabel),sep="\t",quote=F)
