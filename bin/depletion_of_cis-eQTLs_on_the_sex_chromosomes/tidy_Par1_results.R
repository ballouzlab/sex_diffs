...
# Extract PAR1_XXY results

file_list <- list.files(path="PAR1-XXY", pattern="*_par1_significant_correlation_results.tsv", full.name=TRUE, recursive=TRUE)
celltypes <- sub("PAR1-XXY/*", "", file_list) %>% sub("/.*","", .)
...
for (i in 1:nrow(eSNP1_XXY)) {
    ...
    df_xx[[i]] <- fread(sprintf("PAR1-XX/%s/%s_female_par1_correlation_results.tsv", ctype, ctype)) %>%
        filter(geneid==gene) %>% 
        filter(snpid==snp) %>% 
...
for (i in 1:nrow(eSNP1_XXY)) {
    ...
    gene <- eSNP1_XXY$geneid_XXY[i]
    snp <- eSNP1_XXY$snpid_XXY[i]
    df_xy[[i]] <- fread(sprintf("PAR1-XY/%s/%s_male_par1_correlation_results.tsv", ctype, ctype)) %>%
        filter(geneid==gene) %>% 
        filter(snpid==snp) %>% 
        mutate(celltype=ctype) %>%
        data.frame()         
...
fwrite(eSNP1,sprintf("PAR1-XXY/PAR1-XXY_top_eqtls.tsv"),sep="\t",quote=F)

rm(list=ls())

# Extract PAR1_XX results

file_list <- list.files(path="PAR1-XX", pattern="*_par1_significant_correlation_results.tsv", full.name=TRUE, recursive=TRUE)
...
for (i in 1:nrow(eSNP1_XX)) {
    ...
    gene <- eSNP1_XX$geneid_XX[i]
    snp <- eSNP1_XX$snpid_XX[i]
    df_xxy[[i]] <- fread(sprintf("PAR1-XXY/%s/%s_joint_par1_correlation_results.tsv", ctype, ctype)) %>%
        filter(geneid==gene) %>% 
        filter(snpid==snp) %>% 
        mutate(celltype=ctype) %>%
        data.frame()         
...
fwrite(eSNP1,sprintf("PAR1-XX/PAR1-XX_top_eqtls.tsv"),sep="\t",quote=F)

rm(list=ls())

# Extract PAR1_XY results

file_list <- list.files(path="PAR1-XY", pattern="*_par1_significant_correlation_results.tsv", full.name=TRUE, recursive=TRUE)
...
fwrite(eSNP1,sprintf("PAR1-XY/PAR1-XY_top_eqtls.tsv"),sep="\t",quote=F)