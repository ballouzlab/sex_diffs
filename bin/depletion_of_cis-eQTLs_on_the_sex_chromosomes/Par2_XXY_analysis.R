...
female_gene_snp_pairs <- fread(sprintf("%s/PAR2-XX/%s/%s_female_par2_gene-snp_pairs.tsv", output, cellLabel, cellLabel))
male_gene_snp_pairs <- fread(sprintf("%s/PAR2-XY/%s/%s_male_par2_gene-snp_pairs.tsv", output,cellLabel,cellLabel))
gene_snp_df <- inner_join(female_gene_snp_pairs,male_gene_snp_pairs)
...
out.dir = paste0(output,'/PAR2-XXY/')
dir.create(out.dir)
out.dir2 = paste0(out.dir,"/",cellLabel)
dir.create(out.dir2)

# Extract PAR X region genes 
log_exprs_df_par1 <- log_exprs_df %>% select("sampleid",colnames(log_exprs_df)[(colnames(log_exprs_df) %in% gene_snp_df$geneid) == TRUE])
dim(log_exprs_df_par1)

fwrite(log_exprs_df_par1,paste0(out.dir2,"/", cellLabel,'_joint_par2_expression.tsv'),sep="\t")

fwrite(gene_snp_df,paste0(out.dir2,"/", cellLabel,'_joint_par2_gene-snp_pairs.tsv'),sep="\t")

gene_ids <- unique(gene_snp_df$geneid)
...
fwrite(gene_snp_df,paste0(out.dir2,"/", cellLabel,'_joint_par2_log_residuals.tsv'),sep="\t")

genotype_df <- genotype_df %>% filter(sampleid %in% log_exprs_df_par1$sampleid)

fwrite(genotype_df,paste0(out.dir2,"/", cellLabel,'_joint_par2_genotypes.tsv'),sep="\t")
...
fwrite(log_spearman_df,sprintf("%s/%s_joint_par2_correlation_results.tsv", out.dir2, cellLabel),sep="\t",quote=F)
...
fwrite(log_spearman_df_significant,sprintf("%s/%s_joint_par2_significant_correlation_results.tsv", out.dir2, cellLabel),sep="\t",quote=F)
