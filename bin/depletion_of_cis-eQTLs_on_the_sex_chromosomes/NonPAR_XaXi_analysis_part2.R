...
input.dir = paste0(output,'/NonPAR-XaXi/', cellLabel)

# Input files
log_residual_df <- fread(paste0(input.dir,"/", cellLabel,'_nonPAR_XaXi_log_residuals.tsv'))
genotype_df <- fread(paste0(input.dir,"/", cellLabel,'_nonPAR_XaXi_genotypes.tsv'))
gene_snp_df <- fread(paste0(input.dir,"/", cellLabel,'_nonPAR_XaXi_gene-snp_pairs.tsv'))
...
for (i in 1:4) {
    ...
    fwrite(log_spearman_df,sprintf("%s/%s_nonPAR_XaXi_correlation_results_%s.tsv", input.dir, cellLabel,i),sep="\t",quote=F)
}
