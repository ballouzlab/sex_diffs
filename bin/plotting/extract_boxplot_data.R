...
output <- set_output("2022-12-08", "expr-geno-plots")
...
get_expressions <- function (x) {
    # x="B_memory"
    cDir <-"/directflow/SCCGGroupShare/projects/SeyhanYazar/onek1k/sex_specific_eQTL_analysis/results"
    #expression_filename <- sprintf("%s/round1/%s_expression.tsv", cDir, cellLabel)
    # For females:
    expression_filename_f <- sprintf("%s/2022-10-12_female-only/%s/round1/%s_chr%s_log_residuals_female.tsv", cDir,x,x, chr)
    ...
    expression_df2_f<-setNames(data.frame(matrix(ncol = 3, nrow = 0)), c(genename, "sampleid", "sex")) 
     ...
     # For males:
    expression_filename_m <- sprintf("%s/2022-09-27_male-only/%s/round1/%s_chr%s_log_residuals_male.tsv",cDir, x,x, chr)
    ...
    # For joint:
    expression_filename_j <- sprintf("/directflow/SCCGGroupShare/projects/SeyhanYazar/onek1k/cell_specific_eQTL_analysis_June21/results/2021-08-16_conditional-analysis/%s/round1/%s_chr%s_log_residuals.tsv",x,x, chr)
...
combine_expressions <- Filter(function(x) dim(x)[1] > 0, combine_expressions)
...
# expr_geno_df$genotype <- factor(expr_geno_df$genotype, labels=c(paste0(A1,A1), paste0(A1,A2)))
