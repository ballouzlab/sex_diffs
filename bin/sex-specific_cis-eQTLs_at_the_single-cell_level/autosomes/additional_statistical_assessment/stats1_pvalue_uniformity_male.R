...
#    # SNP="10:71993094_G"
#    chr=sub(":.*","",SNP)
#    filename <- sprintf("/directflow/SCCGGroupShare/projects/SeyhanYazar/onek1k/sex_specific_eQTL_analysis/results/2022-03-14_conditional-analysis-stratified/%s/round1/%s_chr%s_correlation_results_male.tsv", celltype1, celltype1, chr)
...
# fwrite(pvalues_df, sprintf("/directflow/SCCGGroupShare/projects/SeyhanYazar/onek1k/sex_specific_eQTL_analysis/results/2022-07-05_pi-analysis/p_outputs/female_vs_male_%s_pvalues.tsv", celltype1), quote=F, sep="\t")

### Reverse order of the groups 
# Exract male results that are not in the joint then female outputs
not_in_joint_df <- male_results_df[!male_results_df$snpid %in% joint_results_df$snpid,]
dim(not_in_joint_df)
not_in_female_df <-not_in_joint_df[!not_in_joint_df$snpid %in% female_results_df$snpid,]
not_in_female_df <- not_in_female_df %>% select(geneid, snpid)
dim(not_in_female_df)
head(not_in_female_df)
# 
extract_info <- function (x,y) {
     GENE=y$geneid
     SNP=y$snpid
     ...
     # SNP="10:71993094_G"
     chr=sub(":.*","",SNP)
     filename <- sprintf("/directflow/SCCGGroupShare/projects/SeyhanYazar/onek1k/sex_specific_eQTL_analysis/results/2022-10-12_female-only/%s/round1/%s_chr%s_correlation_results_female.tsv", celltype1, celltype1, chr)
     df_original_eqtl <- fread(filename)
     df_original_eqtl <- df_original_eqtl %>% filter(snpid==SNP & geneid==GENE)
     df_original_eqtl %>% select(-c("geneid", "snpid"))
}
 
pvalues_df <- not_in_female_df %>% group_by(snpid,geneid) %>% group_modify(extract_info)
# 
fwrite(pvalues_df, sprintf("/directflow/SCCGGroupShare/projects/SeyhanYazar/onek1k/sex_specific_eQTL_analysis/results/2022-10-18_pi-analysis/p_outputs/male_vs_female_%s_pvalues.tsv", celltype1), quote=F, sep="\t")
# 