...
# Set output ------------------------------------------
output <- set_output("2022-09-27", "chr-X-joint-analysis")
...
calculate_residuals <- function (x) {
  ...
  # Generate model
  model <- lm(expression ~ sex + age + pc1 + pc2 + pc3 + pc4 + pf1 + pf2 , data=test_df)
...
fwrite(log_residual_df,sprintf("%s/%s_chrX_log_residuals.tsv", out.dir, cellLabel),sep="\t",quote=F)
...
fwrite(log_spearman_df,sprintf("%s/%s_chrX_round1_correlation_results.tsv", out.dir, cellLabel),sep="\t",quote=F)
...
fwrite(log_spearman_df_significant,sprintf("%s/%s_chrX_round1_significant_correlation_results.tsv", out.dir, cellLabel),sep="\t",quote=F)
