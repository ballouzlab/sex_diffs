...
# Set output ------------------------------------------
output <- set_output("2022-03-14", "conditional-analysis-stratified")
...
# Input filenames
expression_filename <- sprintf("%s/outputs/filtered_matrices/%s_expression_genes_removed.tsv", data.dir, cellLabel)
...
covariate_filename <- sprintf("%s/outputs/peer_factors/%s_peer_factors_after_removing_genes.tsv", data.dir, cellLabel)
...
fwrite(gene_snp_df,sprintf("%s/%s_chrX_gene_SNP_pairs.tsv", out.dir, cellLabel),sep="\t",quote=F)
...
calculate_residuals <- function (x) {
  ...
  # Generate model
  model <- lm(expression ~ sex + pc1 + pc2 + pc3 + pc4 + age + pf1 + pf2 , data=test_df)
  df_res <- covariate_df %>% select(sampleid)
  resid(model)
  # df_res$residuals=resid(model)
  # colnames(df_res)[2] <- gene
  # df_res
}
...
log_residual_df$sampleid <- rownames(log_residual_df)
print(log_residual_df[1:5,1:5])
dim(log_residual_df)

fwrite(log_residual_df,sprintf("%s/round1/%s_chr23_log_residuals_female.tsv", out.dir, cellLabel),sep="\t",quote=F)
...
snpLoc_df$locid <- paste0("23:",snpLoc_df$pos,"_",snpLoc_df$A2)
snpLoc_df <- snpLoc_df %>% select("snpid", "locid")
log_spearman_df <- left_join(log_spearman_df,snpLoc_df, by="snpid")
log_spearman_df$snpid <- log_spearman_df$locid
log_spearman_df <- log_spearman_df %>% select(-c("locid"))

fwrite(log_spearman_df,sprintf("%s/round1/%s_chr23_correlation_results_female.tsv", out.dir, cellLabel),sep="\t",quote=F)
...
fwrite(log_spearman_df_significant,sprintf("%s/round1/%s_chr23_significant_correlation_results_female.tsv", out.dir, cellLabel),sep="\t",quote=F)
