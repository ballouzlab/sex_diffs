...
celltype = celltypes[i] 
    ...
    m = match( temp@meta.data$individual, pcs[,1]) 
    temp@meta.data = cbind(temp@meta.data, pcs[m,] )
	
    temp <- ScaleData(temp, features = all.genes, vars.to.regress=c("age", "pc1", "pc2", "pc3", "pc4"))
    exprs =  temp@assays$SCT@scale.data
    saveRDS(exprs, file=  paste0("scale_", celltype, ".Rds") )

    test = FindMarkers(temp,  slot="scale.data", ident.1="1", ident.2="2", logfc.threshold=-Inf, test.use="wilcox", group.by="sex")
    save(test, file = paste0("wilcox_DE_reg_", celltype, ".Rdata"))
    
    test = FindMarkers(temp,  slot="scale.data", ident.1="1", ident.2="2", logfc.threshold=-Inf, test.use="MAST", group.by="sex")
    save(test, file = paste0("mast_DE_reg_", celltype, ".Rdata"))
